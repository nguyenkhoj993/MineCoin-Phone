...
#install ccminer
apt -y update && apt -y install git && apt -y upgrade && apt -y install wget && apt -y install build-essential && apt -y install automake && apt-get install libssl-dev && git clone --single-branch -b ARM https://github.com/monkins1010/ccminer.git && cd ./ccminer && chmod +x build.sh && chmod +x configure.sh && chmod +x autogen.sh && ./build.sh
#start config file
